"""M2 end-to-end: submit → upload signal (dormancy gate) → OCR → text persisted.
...
async def test_ocr_happy_path() -> None:
    async with await WorkflowEnvironment.start_time_skipping() as env:
        ...
        async with make_worker(env, repo, store=store):
            ...
            async with _client(app) as ac:
                ...
                # simulate the client PUTting the PDF to the signed URL
                await store.put(f"{claim_id}/source.pdf", b"%PDF-1.7 sample")
                ...
                assert await store.exists(f"{claim_id}/ocr.txt")
                text = (await store.get(f"{claim_id}/ocr.txt")).decode()
...
async def test_ocr_retries_then_succeeds() -> None:
    async with await WorkflowEnvironment.start_time_skipping() as env:
        ...
        settings = Settings(temporal_task_queue=TASK_QUEUE)
        async with make_worker(env, repo, store=store, ocr=flaky):
            app = create_app(repo=repo, temporal_client=env.client, settings=settings)
            async with _client(app) as ac:
                claim_id = (await ac.post("/claims", json={"metadata": META})).json()["claim_id"]
                await store.put(f"{claim_id}/source.pdf", b"%PDF-1.7 sample")
